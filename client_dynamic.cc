...
int main(int argc, char *argv[]) {
  ...
  char *buf = reinterpret_cast<char *>(aligned_alloc(262144, 1073741824L));
  size_t block_num = 1073741824L / kWriteSize;
  int send_count = 0;
  ...
  while (should_infini_loop) {
    ...
    for (int i = 0; i < kTransmitDepth && !c_ctx.que.empty(); i++) {
      ...
      c_ctx.que.pop();
      c_ctx.mr = ibv_reg_mr(c_ctx.dev_info.pd, reinterpret_cast<char *>(buf + send_count * kWriteSize),
                            kWriteSize, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_READ);
      {
        int ret = 0;
        ...
        write_list.addr =
            reinterpret_cast<uintptr_t>(buf + send_count * kWriteSize);
        ...
        send_list.addr = reinterpret_cast<uintptr_t>(
            buf + send_count * kWriteSize + (loc + kRdmaQueueSize / 2) * kWriteSize);
        send_list.length = 4096;
        send_list.lkey = c_ctx.mr->lkey;

        struct ibv_send_wr send_wr;
        memset(&send_wr, 0, sizeof(ibv_send_wr));
        send_wr.wr_id = reinterpret_cast<uintptr_t>(
            c_ctx.remote_addr + (loc + kRdmaQueueSize / 2) * kWriteSize);
